...
#将存好的Excel词频表读取成字典
path='E:/GoodGoodStudyDayDayUp/DayDayNoBug/XiechengComment/词频'  #文件所在文件夹
...
for file in files:
    ...
    wc = wordcloud.WordCloud(
        ...
        max_font_size=500) # 字号最大值
    
    #生成词云图
    wc.generate_from_frequencies(wordFreq) # 从字典生成词云
    #保存图片到指定文件夹
    wc.to_file("E:/GoodGoodStudyDayDayUp/DayDayNoBug/XiechengComment/词云图\\{}.png".format(file.split("\\")[1][:4]))
    #在notebook中显示词云图
    plt.imshow(wc) # 显示词云
    plt.axis('off') # 关闭坐标轴
    plt.show() # 显示图像